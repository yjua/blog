...
即 2^x^ = n ; x = log~2~n 。所以时间复杂度为 O(log~2~n)。
...
则复杂度为 log~3~n 。而对数是可以相互转换底数的。 log~3~n = log~3~2 * log~2~n 。而log~3~2 是常量，我们可以忽略。所以我们一般忽略底数，统一记为 logn。
